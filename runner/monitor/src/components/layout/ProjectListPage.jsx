...
import { Bell, Unlock, Lock, Settings, Plus, Folder, BellOff, Server, Gauge } from 'lucide-react'
...
export default function ProjectListPage({
  ...
  const [addProjectModal, setAddProjectModal] = useState({
    ...
    localProjectId: '', localProjectPath: '', localCreateDirectory: true, localPreparing: false,
    githubAvailable: null, githubError: '',
  })
  ...
  const resetAddProjectModal = () => {
    setAddProjectModal({
      step: null, githubUrl: '', projectId: null, projectPath: null,
      hasSpec: false, specContent: null, whatToBuild: '', successCriteria: '',
      updateSpec: false, error: null, repoMode: 'local',
      orgs: [], repos: [], selectedOrg: '', selectedRepo: '', orgsLoading: false, reposLoading: false,
      inputMode: 'url', newRepoName: '', newRepoPrivate: false, newRepoDescription: '', creatingRepo: false,
      localProjectId: '', localProjectPath: '', localCreateDirectory: true, localPreparing: false,
      githubAvailable: null, githubError: '',
      budgetPer24h: 40,
    })
  ...
  const fetchReposForOrg = (org) => {
    ...
    fetch(`/api/github/repos?owner=${encodeURIComponent(org)}`)
      ...
      .then(data => {
        ...
        setAddProjectModal(prev => ({ ...prev, repos: data.repos || [], reposLoading: false, githubError: '' }))
      })
      .catch((err) => {
        setAddProjectModal(prev => ({ ...prev, reposLoading: false, githubError: err.message }))
      })
  }

  const loadGithubOrgs = () => {
    setAddProjectModal(prev => ({ ...prev, orgsLoading: true, orgs: [], repos: [], selectedOrg: '', selectedRepo: '', githubError: '' }))
    return fetch('/api/github/orgs')
      .then(r => r.json().then(data => ({ ok: r.ok, data })))
      .then(({ ok, data }) => {
        if (!ok) throw new Error(data.error || 'Failed to load GitHub account')
        if (data.available === false) {
          setAddProjectModal(prev => ({
            ...prev,
            orgsLoading: false,
            githubAvailable: false,
            githubError: data.error || 'GitHub CLI is not configured.',
            inputMode: 'url',
          }))
          return false
        }
  ...
  const prepareLocalProject = async () => {
    const { localProjectId, localProjectPath, localCreateDirectory } = addProjectModal
    if (!localProjectId.trim()) return
    setAddProjectModal(prev => ({ ...prev, localPreparing: true, error: null }))
    try {
      const res = await authFetch('/api/local-projects/prepare', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          id: localProjectId.trim(),
          path: localProjectPath.trim() || null,
          createDirectory: localCreateDirectory,
        })
      })
      ...
      setAddProjectModal(prev => ({ ...prev, localPreparing: false, error: err.message }))
    }
  }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-6 sm:mb-8">
          <div className="flex items-start sm:items-center justify-between gap-2">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-neutral-800 dark:text-neutral-100">HPC Agent</h1>
              <p className="text-neutral-500 dark:text-neutral-400 text-sm mt-1 hidden sm:block">HPC-aware AI agent orchestrator</p>
            </div>
            <div className="flex items-center gap-3">
              <button
                onClick={() => navigate('/hpc/endpoints')}
