...
import { agentLabel } from '@/utils/agent-label'

export default function AgentSettingsModal({ agentSettingsModal, setAgentSettingsModal, saveAgentSettings }) {
  const agent = agentSettingsModal.agent
  const currentLabel = agent ? agentLabel(agent) : ''
  return (
    <Modal open={agentSettingsModal.open} onClose={() => setAgentSettingsModal({ ...agentSettingsModal, open: false })}>
      <ModalHeader onClose={() => setAgentSettingsModal({ ...agentSettingsModal, open: false })}>
        ...
        <span>{currentLabel}</span> Settings
      </ModalHeader>
      <ModalContent>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-neutral-600 dark:text-neutral-300 mb-1">Display name</label>
            <input
              type="text"
              ...
              value={agentSettingsModal.displayName ?? ''}
              onChange={(e) => setAgentSettingsModal(prev => ({ ...prev, displayName: e.target.value }))}
              placeholder={agent?.role || 'Custom name'}
            />
            <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-1">
              Shown in the UI instead of the default role. Leave empty to use “{agent?.role || agent?.name}”.
            </p>
          </div>
          <div>
