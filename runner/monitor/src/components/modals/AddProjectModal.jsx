        ...
        {(addProjectModal.step === 'url' || addProjectModal.step === 'cloning') && (
          <div className="space-y-4">
            ...
            <div className="flex rounded-md overflow-hidden border border-neutral-300 dark:border-neutral-600">
              <button
                className={`flex-1 px-3 py-2 text-sm font-medium transition-colors ${addProjectModal.repoMode === 'local' ? 'bg-neutral-800 text-white dark:bg-neutral-200 dark:text-neutral-900' : 'bg-white text-neutral-600 dark:bg-neutral-800 dark:text-neutral-400 hover:bg-neutral-50 dark:hover:bg-neutral-700'}`}
                onMouseDown={(e) => { e.preventDefault(); setAddProjectModal(prev => ({ ...prev, repoMode: 'local', selectedRepo: '', error: null })); }}
              ...
              <button
                ...
                onMouseDown={(e) => { e.preventDefault(); setAddProjectModal(prev => ({ ...prev, repoMode: 'existing', inputMode: 'url', selectedRepo: '', error: null })); }}
              ...
              <button
                ...
                onMouseDown={(e) => {
                  e.preventDefault()
                  setAddProjectModal(prev => ({ ...prev, repoMode: 'new', selectedRepo: '', error: null }))
                  if (addProjectModal.githubAvailable !== true) loadGithubOrgs()
                }}
              >Create GitHub</button>
            </div>

            {addProjectModal.repoMode === 'local' ? (
              <>
                <div className="p-3 rounded border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950 text-sm text-blue-700 dark:text-blue-300">
                  Local projects do not require GitHub CLI or a GitHub account.
                </div>
                <div>
                  <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Project ID</label>
                  <input
                    type="text"
                    ...
                    value={addProjectModal.localProjectId}
                    onChange={(e) => setAddProjectModal(prev => ({ ...prev, localProjectId: e.target.value, error: null }))}
                    disabled={addProjectModal.localPreparing}
                  />
                  <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">Use letters, numbers, dots, underscores, and dashes.</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Project Path <span className="text-neutral-400 font-normal">(optional)</span></label>
                  <input
                    type="text"
                    placeholder="Leave blank for ~/.hpc_agent/local/<project-id>/repo"
                    className="w-full px-3 py-2 border rounded-md bg-white dark:bg-neutral-800 border-neutral-300 dark:border-neutral-600 text-neutral-900 dark:text-neutral-100"
                    value={addProjectModal.localProjectPath}
                    onChange={(e) => setAddProjectModal(prev => ({ ...prev, localProjectPath: e.target.value, error: null }))}
                    disabled={addProjectModal.localPreparing}
                  />
                </div>
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    id="localCreateDirectory"
                    checked={addProjectModal.localCreateDirectory}
                    onChange={(e) => setAddProjectModal(prev => ({ ...prev, localCreateDirectory: e.target.checked }))}
                  />
                  <label htmlFor="localCreateDirectory" className="text-sm text-neutral-700 dark:text-neutral-300">Create directory if it does not exist</label>
                </div>
                <div className="flex justify-end gap-2">
                  <Button variant="outline" onClick={resetAddProjectModal}>Cancel</Button>
                  <Button onClick={prepareLocalProject} disabled={!addProjectModal.localProjectId.trim() || addProjectModal.localPreparing}>
                    {addProjectModal.localPreparing ? <><RefreshCw className="w-4 h-4 mr-2 animate-spin" />Preparing...</> : 'Next'}
                  </Button>
                </div>
              </>
            ) : addProjectModal.repoMode === 'new' ? (
              <>
                {addProjectModal.githubError && (
                  <div className="p-3 rounded border border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-950 text-sm text-yellow-700 dark:text-yellow-300">
                    {addProjectModal.githubError}
                  </div>
                )}
                <div>
                ...
                <div className="flex justify-end gap-2">
                  <Button variant="outline" onClick={resetAddProjectModal}>Cancel</Button>
                  <Button onClick={createNewRepo} disabled={!addProjectModal.selectedOrg || !addProjectModal.newRepoName.trim() || addProjectModal.creatingRepo}>
              ...
              <>
                ...
                <p className="text-xs text-neutral-500 dark:text-neutral-400">
                  Or <button className="underline hover:text-neutral-700 dark:hover:text-neutral-300" onClick={() => {
                    setAddProjectModal(prev => ({ ...prev, inputMode: 'dropdown' }))
                    if (addProjectModal.githubAvailable !== true) loadGithubOrgs()
                  }}>select from your GitHub account</button>
                </p>
              </>
            )}
        ...
        {addProjectModal.step === 'spec' && (
          <div className="space-y-4">
            <div className="p-3 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded text-green-700 dark:text-green-300 text-sm">
              Project prepared: <span className="font-mono font-bold">{addProjectModal.projectId}</span>
            </div>
