#!/usr/bin/env node
import { proxyHpcRequest, isHpcRequest } from './hpc-proxy.js';
/**
...
class ProjectRunner {
  ...
  async runAgent(agent, config, mode = null, task = null, visibility = null) {
    ...
    let hpcDefaults = null;
    if (hpcCfg?.endpoint_uuid) {
      hpcDefaults = { endpoint_id: hpcCfg.endpoint_uuid, account: hpcCfg.account || null, available_partitions: [] };
      try {
        const hpcBackend = process.env.HPC_BACKEND_URL || 'http://localhost:5328';
        const epResp = await fetch(`${hpcBackend}/api/list_all_endpoints`);
        if (epResp.ok) {
          const eps = await epResp.json();
          const ep = Array.isArray(eps) ? eps.find(e => e.endpoint_uuid === hpcCfg.endpoint_uuid) : null;
          if (ep) hpcDefaults.available_partitions = ep.partitions || [];
        }
      } catch { /* non-fatal — agent will see no partition hint */ }
    }
...
const server = http.createServer(async (req, res) => {
  if (isHpcRequest(req.url)) {
    const r = await proxyHpcRequest(req);
  ...
  if (pathParts[0] === 'api' && pathParts[1] === 'projects' && pathParts[2]) {
    ...
    // POST /api/projects/:id/hpc — update project HPC endpoint config
    if (req.method === 'POST' && subPath === 'hpc') {
      ...
      req.on('end', () => {
        try {
          const { endpoint_uuid, account } = JSON.parse(body);
          const cfgPath = runner.configPath;
          const existing = fs.existsSync(cfgPath) ? yaml.load(fs.readFileSync(cfgPath, 'utf-8')) || {} : {};
          if (endpoint_uuid) {
            existing.hpc = { endpoint_uuid };
            if (account) existing.hpc.account = account;
          } else {
            delete existing.hpc;
          }
