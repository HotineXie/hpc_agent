import { getHpcToolDefinition, executeHpcSubmit } from './hpc-tool.js';
/**
 * Agent Runner - Multi-provider agent loop for HPC Agent
 *
...
// ---------------------------------------------------------------------------
function getToolDefinitions(hpcDefaults = null) {
  return [
    {
      name: 'Bash',
    ...
    getHpcToolDefinition(hpcDefaults || {}),
  ];
}
...
async function executeToolDetailed(toolName, toolInput, cwd, remainingMs = 0, bashEnv = null, runtime = null, allowedRepo = null, allowedPaths = null, issuePolicy = null, hpcDefaults = null) {
  switch (toolName) {
    case 'HpcSubmit': {
      const hpcOut = await executeHpcSubmit(toolInput, hpcDefaults || {});
      const ok = !(hpcOut.startsWith('[HpcSubmit Error]') || hpcOut.startsWith('[HpcSubmit Blocked]'));
      return { output: hpcOut, exitCode: null, ok };
    }
...
async function executeTool(toolName, toolInput, cwd, remainingMs = 0, bashEnv = null, runtime = null, allowedRepo = null, allowedPaths = null, issuePolicy = null, hpcDefaults = null) {
  const result = await executeToolDetailed(toolName, toolInput, cwd, remainingMs, bashEnv, runtime, allowedRepo, allowedPaths, issuePolicy, hpcDefaults);
...
export async function runAgentWithAPI(opts) {
  ...
  const canonicalTools = getToolDefinitions(hpcDefaults);
    ...
    for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
      ...
      if (response.stopReason === 'tool_use') {
        ...
        for (const tc of response.toolCalls) {
          ...
          const normalized = await executeToolDetailed(tc.name, tc.input, cwd, remainingMs, bashEnv, runtime, allowedRepo, allowedPaths, issuePolicy, hpcDefaults);
